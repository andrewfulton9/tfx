...
def _is_input_event(event):
  ...
  return event.type in [
      metadata_store_pb2.Event.DECLARED_INPUT, metadata_store_pb2.Event.INPUT
  ]
...
class ReadonlyMetadataStore:
  ...
  def get_artifact_df(self, artifact_id):
    ...
    return (self.get_df_from_single_artifact_or_execution(artifacts[0])
  ...
  def get_execution_df(self, execution_id):
    ...
    return (self.get_df_from_single_artifact_or_execution(executions[0])
  ...
  def get_source_artifact_of_type(self, artifact_id, source_type_name):
    ...
    for a_event in a_events:
      if _is_input_event(a_event):
        continue
      [execution
      ] = self.metadata_store.get_executions_by_id([a_event.execution_id])
      e_events = self.metadata_store.get_events_by_execution_ids([execution.id])
      for e_event in e_events:
        if _is_output_event(e_event):
          continue
        [artifact
        ] = self.metadata_store.get_artifacts_by_id([e_event.artifact_id])
        [artifact_type
  ...
  def get_execution_for_output_artifact(self, artifact_id, type_name):
    ...
    for a_event in a_events:
      ...
      [execution_type
      ] = self.metadata_store.get_execution_types_by_id([execution.type_id])
  ...
  def display_artifact_and_execution_properties(self, artifact_id,
    ...
    artifact_df, execution_df = (self.get_artifact_df(artifact_id),
                                 self.get_execution_df(execution_id))
